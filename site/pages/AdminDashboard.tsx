...
import { SiteContent, Video, PageContent, JobPosting, Article, Product, JobApplication } from '../types';
import { saveSiteContent, getArticles, saveArticle, deleteArticle, getProducts, saveProduct, deleteProduct, getJobApplications, updateApplicationStatus } from '../services/firebase';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ siteContent, setSiteContent }) => {
    const [activeTab, setActiveTab] = useState<'articles' | 'store' | 'writers' | 'lander' | 'pages' | 'careers' | 'applications'>('articles');
    ...
    // Load applications when switching to that tab
    useEffect(() => {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="min-h-[600px] animate-fade-in">
                    ...
                    {activeTab === 'applications' && (
                        <div>
                            <div className="flex justify-between mb-8 items-center">
                                <h2 className="font-serif text-3xl">Job Applications</h2>
                                <button onClick={loadApplications} className="text-sm font-bold uppercase underline">Refresh</button>
                            </div>

                            {applications.length === 0 ? (
                                <div className="bg-white p-12 text-center border border-gray-200">
                                    <p className="font-serif text-xl text-gray-400">No applications received yet.</p>
                                </div>
                            ) : (
                                <div className="bg-white border border-gray-200">
                                    {applications.map(app => (
                                        <div key={app.id} className="border-b border-gray-100 last:border-0">
                                            <div className="p-6 cursor-pointer hover:bg-gray-50" onClick={() => setExpandedAppId(expandedAppId === app.id ? null : app.id)}>
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <h4 className="font-serif text-xl mb-1">{app.applicantName}</h4>
                                                        <div className="flex items-center gap-4 text-xs font-sans text-gray-500">
                                                            <span className="font-bold uppercase tracking-wider text-vakya-salmon">{app.jobTitle}</span>
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <select
                                                            ...
                                                            className={`text-xs font-bold uppercase tracking-widest px-2 py-1 rounded border ${app.status === 'new' ? 'bg-blue-50 text-blue-600 border-blue-200' :
                                                        ...
                                                        <svg className={`w-5 h-5 text-gray-400 transition-transform ${expandedAppId === app.id ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {expandedAppId === app.id && (
                                                <div className="bg-gray-50 p-6 border-t border-gray-100 animate-fade-in">
                                                    <div className="grid md:grid-cols-2 gap-8 mb-6">
                                                        <div>
                                                            <h5 className="font-sans font-bold text-xs uppercase text-gray-400 mb-2">Links</h5>
                                                            <div className="space-y-2 text-sm">
                                                                <a href={app.linkedinUrl} target="_blank" rel="noreferrer" className="block text-blue-600 hover:underline">LinkedIn Profile</a>
                                                                <a href={app.portfolioUrl} target="_blank" rel="noreferrer" className="block text-blue-600 hover:underline">Portfolio / Github</a>
                                                                <a href={app.resumeUrl} target="_blank" rel="noreferrer" className="block text-blue-600 hover:underline">Resume / CV</a>
                                                            </div>
                                                        </div>
                                                        <div>
                                                            <h5 className="font-sans font-bold text-xs uppercase text-gray-400 mb-2">Contact</h5>
                                                            <a href={`mailto:${app.email}`} className="text-lg font-serif underline">{app.email}</a>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <h5 className="font-sans font-bold text-xs uppercase text-gray-400 mb-3">Pitch / Introduction</h5>
                                                        <div className="bg-white p-4 border border-gray-200 rounded text-gray-700 font-serif leading-relaxed whitespace-pre-wrap">
                                                            {app.pitch}
                                                        </div>
                                                    </div>
                                                </div>
